...
export async function POST(request: NextRequest) {
	try {
		...
		const coveredControls = controls.filter(
		...
		const conflictingControls = controls.filter(
		...
		return NextResponse.json({
			...
			coverage: {
				...
				coverage_percentage:
					controlIds.length > 0
						? Math.round(
								((coveredControls + partialControls) / controlIds.length) * 100,
							)
						: 0,
			},
		...
		return NextResponse.json(
			{
				error:
					error instanceof Error ? error.message : "Failed to build coverage",
			},
			{ status: 500 },
		);
	}
