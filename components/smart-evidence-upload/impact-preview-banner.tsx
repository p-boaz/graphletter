...
export function ImpactPreviewBanner({
	...
	return (
		<div
			className="flex items-center gap-3 rounded-lg border border-blue-100 bg-blue-50 p-3"
			data-testid="impact-preview-banner"
		>
			<TrendingUp className="h-5 w-5 shrink-0 text-blue-600" />
			<div className="text-sm">
				<span className="text-blue-800">
					This upload could improve your posture score from{" "}
					<span className="font-semibold" data-testid="impact-preview-score">
						{impact.currentScore.toFixed(1)}%
					</span>{" "}
					to{" "}
					<span className="font-semibold">
						{impact.projectedScore.toFixed(1)}%
					</span>
				</span>
