...
export async function runControlAssessment(
	...
	try {
		...
		if (controlError || !controlData) {
			console.error(`Control query error for ${scfControlId}:`, controlError);
			throw new Error(
		...
		if (maturityError) {
			console.warn(
				`Failed to fetch maturity levels for ${scfControlId}: ${maturityError.message}`,
			);
		}
		...
		const { data: summaryCandidates, error: summaryCandidateError } =
		...
		if (summaryCandidateError) {
			console.warn(
				`Failed to query reusable summary assessments for ${scfControlId}: ${summaryCandidateError.message}`,
			);
		}
		...
		if (existingSummary) {
			...
			const existingMaturityAssessment =
				metadata?.maturity_assessment &&
				typeof metadata.maturity_assessment === "object"
			...
			await appendAIAssessmentLog({
				...
				scfControlId,
				objectiveIds: objectives?.map(
					(objective: AssessmentObjective) => objective.id,
				),
				...
				metadata: {
					...
					overallConfidence: confidenceLevelToScore(
						existingSummary.confidence_level,
					),
			...
			return {
				...
				control_guidance:
					controlData.guidance_micro ||
					controlData.guidance_small ||
					controlData.guidance_medium,
				...
				overall_confidence: confidenceLevelToScore(
					existingSummary.confidence_level,
				),
				summary:
					existingSummary.assessment_summary || "Assessment already completed",
		...
		const qualityCheck = validateObjectiveAssessmentQuality(
			objectiveResults,
			objectives.length,
		);
		if (!qualityCheck.isValid) {
			throw new Error(
				`Quality gate failed for control ${scfControlId}: ${qualityCheck.reason}`,
			);
		}
		...
		const passCount = objectiveResults.filter(
			(r) => r.result === "pass",
		).length;
		...
		if (existingObjectiveError) {
			console.warn(
				`Failed to query reusable objective assessments for ${scfControlId}: ${existingObjectiveError.message}`,
			);
		}

		const existingObjectiveByAoId = new Map<
			string,
			{ id: string; scf_ao_id: string | null }
		>();
		...
		for (const result of objectiveResults) {
			...
			const existingObjective = existingObjectiveByAoId.get(
				objective.scf_ao_id,
			);
			...
			if (assessmentError) {
				console.error(
					`Failed to create assessment for objective ${objective.scf_ao_id}:`,
					assessmentError,
				);
				continue;
			}
		...
		const existingSummaryForRun = (summaryCandidates ?? []).find(
			(candidate) => {
				const metadata =
					candidate.metadata && typeof candidate.metadata === "object"
						? (candidate.metadata as Record<string, unknown>)
		...
		if (existingSummaryForRun?.id) {
			...
			const { data: createdSummaryData, error: summaryError } =
				await serviceSupabase
					.from("assessments")
					.insert({
						user_id: userId,
			...
			if (summaryError) {
				console.error("Failed to create summary assessment:", summaryError);
		...
		console.error(`Assessment failed for control ${scfControlId}:`, error);
		await appendAIAssessmentLog({
			requestId: logContext.requestId,
			sessionId: logContext.sessionId,
			scope: "control_assessment",
			...
			error:
				error instanceof Error
					? error.message
					: "Unknown control assessment error",
		});
		throw error;
	}
}
