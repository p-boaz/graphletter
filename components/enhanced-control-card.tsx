...
import { AlertTriangle, ChevronDown, ChevronUp, Shield, Target, Upload } from "lucide-react";
...
export function EnhancedControlCard({
  ...
  const formattedLastUpdated = lastUpdated ? new Date(lastUpdated).toLocaleDateString() : null;
        ...
        <div className="flex-1 space-y-3">
          ...
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <Badge className={cn("text-xs", gapInfo.color)}>{gapInfo.label}</Badge>
            ...
            <div>
              <h4 className="text-sm font-semibold text-slate-900">{controlTitle}</h4>
              <p className="text-xs text-slate-500">
                ...
                {!hasAssessment
                  ...
                  : assessmentResult
                    ...
                    : ""}
                {formattedLastUpdated ? ` · Updated ${formattedLastUpdated}` : ""}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {maturityLevels && (
              <div className="flex items-center gap-1 rounded-lg bg-ft-cream px-2 py-1 text-xs">
                <Target className="h-3 w-3 text-ft-pink" />
                <span className="text-slate-700">C|P-CMM levels available</span>
              </div>
            )}
          </div>
            ...
            <Collapsible open={isExpanded} onOpenChange={setIsExpanded}>
              <CollapsibleTrigger asChild>
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-auto p-0 text-xs text-ft-black underline hover:text-ft-pink"
                >
                  {isExpanded ? (
                    <>
              ...
              <CollapsibleContent className="mt-3 space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1">
                      {risks.slice(0, 3).map((risk) => (
                        <div key={risk.id} className="rounded border border-red-100 bg-red-50 p-2">
                          ...
                          <p className="mt-1 text-xs font-medium text-red-900">{risk.title}</p>
                          <p className="text-xs text-red-700 line-clamp-2">{risk.description}</p>
                        </div>
                      ))}
                      {risks.length > 3 && (
                    ...
                    <div className="space-y-1">
                      {threats.slice(0, 3).map((threat) => (
                        ...
                        >
                          ...
                          <p className="mt-1 text-xs font-medium text-orange-900">{threat.title}</p>
                ...
                {maturityLevels && (
                  <div className="space-y-2">
                    <h5 className="ft-eyebrow text-[11px]">C|P-CMM Maturity Levels</h5>
