...
export function AssessmentProgressView({
	...
	const totalControlCount = Math.max(
		1,
		liveAssessmentProgress.totalControls ||
			uploadOnlyResult.discovered_controls.length,
	);
		...
		<div className="space-y-6">
			...
			<div className="rounded-lg bg-green-50 p-4 text-center">
				...
				<p className="text-green-700 text-sm">
					Found {uploadOnlyResult.discovered_controls.length} relevant controls
					for &quot;
					...
					<div className="flex items-start gap-3">
						...
						<div className="space-y-1 text-sm">
							<p className="font-medium text-amber-900">
								Graph extraction is limited for this upload
							</p>
							<p className="text-amber-800">
								We could not extract reliable text for graph evidence atoms. AI
								assessment can still run, but graph-native coverage/report
								traceability may be reduced until a text-extractable file is
								uploaded.
							</p>
							{graphExtractionSkipReason ? (
								<p className="font-mono text-[11px] text-amber-700">
									Reason: {graphExtractionSkipReason}
								</p>
							) : null}
						</div>
					</div>
			...
			<Card>
				<CardHeader className="pb-3">
					...
					<CardDescription>
						Status:{" "}
						{uploadOnlyResult.evidence.evidence_status.replace("_", " ")} •
				...
				{assessing ? (
					<div className="mx-auto mt-4 max-w-2xl space-y-3 text-left">
						<div className="rounded-lg border border-blue-200 bg-white p-4">
							...
							<Progress
								value={
									Math.round(
										(liveAssessmentProgress.completedControls /
											totalControlCount) *
											100,
									) || 0
								}
							...
							<div className="mt-2 flex items-center justify-between text-blue-700 text-xs">
								<span>
									...
									{totalControlCount} controls complete
								</span>
								<span>
									{formatEta(liveAssessmentProgress.estimatedRemainingMs)}
								</span>
							</div>
							{processingStage && (
								<p className="mt-2 text-blue-600 text-xs">{processingStage}</p>
							)}
						</div>
					...
					<>
						<p className="mb-4 text-blue-700 text-sm">
							...
							{uploadOnlyResult.discovered_controls.length} relevant SCF
							controls
						</p>
						<Button
					...
					<div className="rounded-lg border border-red-200 bg-red-50 p-4">
						<div className="mb-3 flex items-start gap-2">
							...
							<div>
								...
								<p className="mt-1 text-red-700 text-xs">
									These controls encountered errors during AI assessment and can
									be retried.
								</p>
							</div>
						</div>
						<div className="mb-3 max-h-32 space-y-1 overflow-y-auto">
							{failedControls.map((fc) => (
								...
								>
									<span className="font-mono text-[11px] text-red-900">
										{fc.control_id}
									</span>
									<span className="max-w-[200px] truncate text-[10px] text-red-600">
										{fc.error}
									</span>
								</div>
							))}
