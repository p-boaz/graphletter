...
const log = createLogger("api/controls/framework-impact");
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!controlIds || controlIds.length === 0) {
      return NextResponse.json({ error: "controlIds array is required" }, { status: 400 });
    }
    ...
    log.error("controls.framework_impact.unhandled", {
      detail: error instanceof Error ? error.message : String(error),
    });
    return NextResponse.json({ error: "Failed to compute framework impact" }, { status: 500 });
  }
}
