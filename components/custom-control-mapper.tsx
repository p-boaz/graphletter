...
export function CustomControlMapper() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "Failed to analyze policy text");
    } finally {
      ...
      <Card>
        <CardHeader className="space-y-1">
          <div className="flex items-center gap-2">
            <Brain className="h-5 w-5 text-ft-pink" />
            <CardTitle className="text-xl">AI-Powered Control Mapper</CardTitle>
          </div>
          <CardDescription>
            Paste your custom policy or control text below, and our AI will map it to relevant SCF
            controls and show framework coverage.
          </CardDescription>
        </CardHeader>
...
'All user accounts must be protected with multi-factor authentication. Passwords must be at least 12 characters long and include uppercase, lowercase, numbers, and special characters. Account lockouts must occur after 5 failed login attempts. Administrative accounts require additional approval processes and must be reviewed quarterly.'"
            ...
            <div className="flex items-center justify-between text-muted-foreground text-sm">
              ...
              <span>{policyText.split(/\s+/).filter((w) => w.length > 0).length} words</span>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                <div className="space-y-2">
                  <div className="ft-serif font-bold text-2xl text-ft-black">
                    {results.analysis.totalMatches}
                  </div>
                  <div className="text-muted-foreground text-sm">Controls Matched</div>
                </div>
                <div className="space-y-2">
                  <div
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-muted-foreground text-sm">Concepts Found</div>
                </div>
                <div className="space-y-2">
                  <div className="ft-serif font-bold text-2xl text-ft-black">
              ...
              <div className="mt-4 rounded-lg bg-muted p-4">
                <div className="mb-2 font-medium text-sm">Overall Assessment</div>
                <div className="text-sm">{results.analysis.overallAssessment}</div>
              </div>
            </CardContent>
          </Card>
          ...
          <Tabs defaultValue="controls" className="space-y-4">
                ...
                results.analysis.matchedControls.map((control) => (
                  <Card key={control.id}>
                    <CardHeader>
                      <div className="flex items-start justify-between">
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center gap-2">
                            <Badge variant="outline">Domain: {control.domain_id}</Badge>
                            <Badge
                              variant={
                    ...
                    <CardContent className="space-y-3">
                      ...
                      <div>
                        <div className="mb-1 font-medium text-sm">Why This Maps</div>
                        <div className="text-muted-foreground text-sm">{control.reasoning}</div>
                      </div>

                      {control.scf_control_mappings && control.scf_control_mappings.length > 0 && (
                        <div>
                          <div className="mb-2 font-medium text-sm">Framework Mappings</div>
                          <div className="flex flex-wrap gap-1">
                            {control.scf_control_mappings.slice(0, 8).map((mapping, idx) => (
                            ...
                            {control.scf_control_mappings.length > 8 && (
                              <Badge variant="secondary" className="text-xs">
                                +{control.scf_control_mappings.length - 8} more
                              </Badge>
                            )}
                          </div>
                        </div>
            ...
            <TabsContent value="frameworks" className="space-y-4">
              {!results.frameworkCoverage || results.frameworkCoverage.length === 0 ? (
                <Card>
                  <CardContent className="pt-6">
                    <div className="text-center text-muted-foreground">
                ...
                <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                  {results.frameworkCoverage.map((framework, index) => (
                    <Card key={index}>
                      <CardHeader>
                        <CardTitle className="text-lg">{framework.frameworkName}</CardTitle>
                        {framework.frameworkVersion && (
                          <CardDescription>Version: {framework.frameworkVersion}</CardDescription>
                        )}
                      </CardHeader>
                      <CardContent>
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            <span className="font-medium text-sm">Controls Covered</span>
                            <span className="ft-serif font-bold text-ft-black text-lg">
                              {framework.coveredControls}
                            </span>
                          </div>
                          <div className="text-muted-foreground text-xs">
                            Control IDs: {framework.controlIds.slice(0, 5).join(", ")}
                ...
                <Card>
                  ...
                  <CardContent>
                    {results.analysis.concepts.length === 0 ? (
                      <div className="text-muted-foreground text-sm">No concepts identified</div>
                    ) : (
                ...
                <Card>
                  ...
                  <CardContent>
                    {results.analysis.gaps.length === 0 ? (
                      <div className="text-muted-foreground text-sm">No gaps identified</div>
                    ) : (
                      ...
                      <ul className="space-y-1">
                        {rec.actionItems.map((item, itemIndex) => (
                          <li
                            ...
                            className="flex items-start gap-2 text-muted-foreground text-sm"
                          >
                            <span className="mt-2 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-ft-pink" />
                            {item}
                          </li>
                        ))}
                      </ul>
