...
export function createControlRunKey(
	...
	return createHash("sha256")
		.update(`${evidenceId}:${scfControlId}:${evidenceContentHash}`)
		.digest("hex");
}
...
Additionally, please analyze the visual content of the provided image/screenshot for compliance evidence.`
...
export async function generateObjectWithRetry(
	...
	for (let attempt = 1; attempt <= MAX_AI_CALL_ATTEMPTS; attempt += 1) {
		...
		try {
			...
			const errorMessage =
				error instanceof Error ? error.message : "Unknown AI call failure";
	...
	throw lastError instanceof Error
		? lastError
		: new Error(`AI call failed in ${call}`);
}
