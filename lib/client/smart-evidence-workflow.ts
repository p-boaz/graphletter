...
function buildProgressHeaders(sessionId: string | null): Record<string, string> {
...
export function assessExtractedGraphContent(content: string): GraphContentAssessment {
...
export function createSmartEvidenceWorkflowClient(sessionId: string | null) {
  return {
    async runUploadWorkflow(options: UploadWorkflowOptions): Promise<UploadWorkflowResult> {
      ...
      const extractResult = (await handleResponse(extractResponse)) as ExtractContentResult;
      ...
      uploadForm.append("documentation_artifact", options.documentationArtifact);
      ...
      if (options.versionAction === "replace" && options.existingEvidence) {
        ...
        uploadForm.append("new_version", String(options.existingEvidence.version + 1));
      }
      ...
      const uploadResult = (await handleResponse(uploadResponse)) as UploadOnlyResult;
      ...
      uploadResult.graph_extraction = {
        ...
        quality: extractionPayload?.quality || (graphContent.isUsable ? "ready" : "limited"),
        content_length: extractionPayload?.content_length ?? graphContent.contentLength,
        requested_extraction: extractionPayload?.requested_extraction ?? graphContent.isUsable,
        executed_extraction: extractionPayload?.executed_extraction ?? graphContent.isUsable,
        limited_reason:
