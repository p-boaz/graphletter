...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimitResponse = checkRouteRateLimit(IMPACT_RATE_LIMIT, user.id, request.headers);
    ...
    const preview = await previewUploadImpact(supabase, user.id, controlIds, frameworkId || null);
    ...
    return NextResponse.json({ error: "Failed to compute impact preview" }, { status: 500 });
  }
}
