...
function getDisplayName(frameworkName: string): string {
	return (
		FRAMEWORK_DISPLAY_NAMES[frameworkName] || frameworkName.replace(/_/g, " ")
	);
}

// Tier classification for visual emphasis
function getTier(fw: FrameworkImpact): "major" | "notable" | "standard" {
	if (fw.controls_advanced >= 3 || fw.unique_requirements_touched >= 5)
		return "major";
	if (fw.controls_advanced >= 2 || fw.unique_requirements_touched >= 3)
		return "notable";
	return "standard";
}
...
export function FrameworkImpactCascade({
	...
	useEffect(() => {
		...
		const total = showAll
			...
			: Math.min(data.frameworks.length, 12);
	...
	useEffect(() => {
		...
		const timer = setTimeout(() => {
			setDisplayedTotal((c) =>
				Math.min(c + step, data.total_frameworks_impacted),
			);
		}, 60);
	...
	const passCount =
		assessmentResults?.filter((a) => a.overall_result === "pass").length ?? 0;
	const partialCount =
		assessmentResults?.filter((a) => a.overall_result === "partial").length ??
		0;
		...
		return (
			<div className="flex items-center justify-center gap-2 rounded-lg bg-slate-50 p-6">
				...
				<span className="text-slate-600 text-sm">
					Computing cross-framework impact...
				</span>
			</div>
		);
	}
	...
	const displayFrameworks = showAll
		? data.frameworks
		: data.frameworks.slice(0, 12);
