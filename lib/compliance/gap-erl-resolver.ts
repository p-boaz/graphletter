...
export async function resolveGapToErl(
	...
	// Find ERL mappings that cover any of the gap controls
	...
	if (mappingError) {
		throw new Error(`Failed to fetch ERL mappings: ${mappingError.message}`);
	}
	...
	const { data: erlRows, error: erlError } = await supabase
		...
		.select(
			"id, erl_id, documentation_artifact, artifact_description, area_of_focus",
		)
		.in("id", erlIds);
