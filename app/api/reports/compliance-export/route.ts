...
async function resolveControlIds(
  ...
  if (frameworkId) {
    ...
    if (error) throw new Error(error.message);
    return [...new Set(((data || []) as ControlMappingRow[]).map((r) => r.control_id))];
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const framework = await resolveFrameworkFilter(supabase, frameworkId, frameworkName);
    const controlIds = await resolveControlIds(supabase, framework.id, framework.name);
    ...
    for (const mapping of mappings || []) {
      ...
      const sourceEvidenceId = (document?.source_evidence_id as string | null | undefined) ?? null;
      ...
      atomDetailsByControl.get(controlId)?.push({
        ...
        confidence: typeof atom?.confidence === "number" ? (atom.confidence as number) : null,
        source_locator:
          (atom?.source_locator as Record<string, unknown> | null | undefined) ?? null,
        ...
        mapping_polarity: mapping.mapping_polarity === "contradicts" ? "contradicts" : "supports",
        coverage_rank: coverageStrengthRank((mapping.coverage_strength as string) || "none"),
      });
    }
    ...
    if (sourceEvidenceError) {
      return NextResponse.json({ error: sourceEvidenceError.message }, { status: 500 });
    }

    const evidenceTypeById = new Map(
      (sourceEvidenceRows || []).map((row: { id: string; evidence_type: string }) => [
        row.id,
    ...
    for (const [controlId, atomDetails] of atomDetailsByControl.entries()) {
      atomDetailsByControl.set(
        ...
        atomDetails.map((atom) => ({
          ...atom,
          evidence_type:
            (atom.source_evidence_id ? evidenceTypeById.get(atom.source_evidence_id) : null) ||
    ...
    const controlReports = computedGaps.map((gap) => {
      ...
      const evidenceTypeBreakdown = countBy(relatedAtoms.map((atom) => atom.evidence_type));
      const atomTypeBreakdown = countBy(relatedAtoms.map((atom) => atom.atom_type || "other"));
    ...
    for (const controlReport of controlReports) {
      for (const [type, count] of Object.entries(controlReport.evidence_type_breakdown)) {
        globalEvidenceTypeBreakdown[type] = (globalEvidenceTypeBreakdown[type] || 0) + count;
      }
      for (const [type, count] of Object.entries(controlReport.atom_type_breakdown)) {
        globalAtomTypeBreakdown[type] = (globalAtomTypeBreakdown[type] || 0) + count;
      }
    }
    ...
    const summary = {
      ...
      conflicting: controlReports.filter((r) => r.status === "conflicting").length,
      traceable_statuses: controlReports.filter((r) => r.status === "missing" || r.traceable)
        .length,
    };
    ...
    return apiError("reports.compliance_export_failed", "Failed to export report", 500, error);
  }
}
