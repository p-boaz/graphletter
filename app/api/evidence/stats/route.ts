...
export async function GET(request: NextRequest) {
	try {
		...
		if (evidenceError) {
			console.error("Error fetching evidence stats:", evidenceError);
		...
		const uniqueEvidenceGroups = [
			...new Set(evidence.map((e) => e.evidence_group_id || e.id)),
		];
		const stats = {
		...
		evidence.forEach((item) => {
			...
			stats.by_type[item.evidence_type] =
				(stats.by_type[item.evidence_type] || 0) + 1;

			// Control counts
			stats.by_control[item.scf_control_id] =
				(stats.by_control[item.scf_control_id] || 0) + 1;
		...
		stats.pending_review =
			stats.by_status.submitted + stats.by_status.under_review;
		stats.approval_rate =
			submittedCount > 0
		...
		if (erlError) {
			console.warn("Error fetching ERL requirements:", erlError);
		}
		...
		if (erlRequirements) {
			...
			(erlRequirements as ERLRequirementRow[]).forEach((req) => {
				if (controlId) {
					...
					if (
						req.scf_control_evidence_mappings &&
						req.scf_control_evidence_mappings.length > 0
					) {
						requiredErls.add(req.erl_id);
						if (stats.by_erl[req.erl_id] > 0) {
							coveredErls.add(req.erl_id);
			...
			coverage.coverage_percentage =
				coverage.total_requirements > 0
					? Math.round(
							(coverage.covered_requirements / coverage.total_requirements) *
								100,
						)
					: 0;
		}
		...
		console.error("Error in evidence stats GET:", error);
		return NextResponse.json(
