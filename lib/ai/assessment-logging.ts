...
export type AIAssessmentLogScope =
	| "ai_call"
	| "control_assessment"
	| "retry"
	| "timeout";
...
export async function appendAIAssessmentLog(
	...
	try {
		...
		console.error("Failed to append AI assessment log entry:", error);
	}
...
export async function readAIAssessmentLogs(
	...
	try {
		...
		const filtered = parsed.filter((entry) => {
			...
			if (
				evidenceContentHash &&
				entry.evidenceContentHash !== evidenceContentHash
			)
				return false;
		...
		console.error("Failed to read AI assessment logs:", error);
		return [];
	}
}
