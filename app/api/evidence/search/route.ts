...
async function runEvidenceSearch(
  ...
  const normalizedLimit = Math.min(200, Math.max(1, Number(params.limit) || 50));
  ...
  let searchQuery = supabase.from("evidence").select(baseSelect).eq("user_id", userId);
  ...
  const highlightedResults = typedEvidence.map((item: EvidenceSearchRow) => {
    ...
    const contextEnd = Math.min(content.length, matchIndex + params.query.length + 100);
    return {
      ...item,
      search_highlight: {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const payload = await runEvidenceSearch(supabase, user.id, {
      query,
      searchType: searchType === "content" || searchType === "metadata" ? searchType : "all",
      includeContent,
      limit,
      offset,
      status,
    ...
    return apiError("evidence.search_failed", "Search failed", 500, error);
  }
}
