...
export function getControlOverallConfidence(objectives: ControlObjective[]) {
	...
	return (
		objectives.reduce((sum, obj) => sum + obj.confidence, 0) / objectives.length
	);
}
...
export function getTopGapAndRecommendation(objectives: ControlObjective[]) {
	...
	for (const objective of objectives) {
		...
		if (!topRecommendation && objective.recommendations?.length) {
			topRecommendation =
				objective.recommendations.find((rec) => rec.trim().length > 0) ?? null;
		}
	...
	return assessments.reduce(
		(groups, assessment) => {
			...
			if (assessment.completed_at) {
				groups[controlId].completedAtByAssessmentId[assessment.id] =
					assessment.completed_at;
			}

			if (assessment.linked_evidence && assessment.linked_evidence.length > 0) {
				groups[controlId].evidenceByAssessmentId[assessment.id] =
