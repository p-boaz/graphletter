...
test.describe("compliance autopilot — Phase 4", () => {
  test("compliance inbox page loads and displays prioritized action items", async ({
    ...
    try {
      ...
      await expect(page.getByTestId(selectors.inbox.postureSummary)).toBeVisible();
      await expect(page.getByTestId(selectors.inbox.postureSummary)).toContainText("72.5%");
  ...
  test("evidence page shows freshness health dots", async ({ page }, testInfo) => {
    const observer = inspect_console_errors(page);
    let report = observer.getReport();
    ...
    try {
      ...
      await expect(page.getByTestId(selectors.evidence.row).first()).toBeVisible({
        timeout: 10_000,
      });
  ...
  test("impact preview banner shows score improvement in upload wizard", async ({
    ...
    try {
      ...
      await expect(page.getByTestId(selectors.impactPreview.banner)).toBeVisible({
        timeout: 10_000,
      });

      // Verify score improvement text
      await expect(page.getByTestId(selectors.impactPreview.banner)).toContainText("72.5%");
      await expect(page.getByTestId(selectors.impactPreview.banner)).toContainText("+5.8%");
    } finally {
      observer.stop();
      report = observer.getReport();
      await trace_failure(testInfo, report);
  ...
  test("dashboard shows inbox summary with top action items", async ({ page }, testInfo) => {
    const observer = inspect_console_errors(page);
    let report = observer.getReport();
    ...
    try {
      ...
      await expect(page.getByTestId(selectors.inbox.summaryCard)).toContainText("Compliance Inbox");

      // Should contain at least one action item
      await expect(page.getByTestId(selectors.inbox.summaryCard)).toContainText("Expired");

      // Should have a "View all" link
      const viewAll = page.getByTestId(selectors.inbox.summaryCard).getByText("View all");
  ...
  test("compliance inbox shows empty state when no actions", async ({ page }, testInfo) => {
    const observer = inspect_console_errors(page);
    let report = observer.getReport();
    ...
    try {
      ...
      await expect(page.getByTestId(selectors.inbox.emptyState)).toContainText("All clear");
    } finally {
      observer.stop();
      report = observer.getReport();
      await trace_failure(testInfo, report);
