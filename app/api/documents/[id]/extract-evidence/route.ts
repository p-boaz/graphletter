	...
	try {
		...
		if (document.source_evidence_id) {
			const { data: evidence } = await supabase
				.from("evidence")
				.select(
					"scf_control_id, extracted_content, processed_content, evidence_data",
				)
		...
		return NextResponse.json(
			{
				error:
					error instanceof Error ? error.message : "Failed to extract evidence",
			},
			{ status: 500 },
		);
	}
