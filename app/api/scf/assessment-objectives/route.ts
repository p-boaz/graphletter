...
export async function GET(request: NextRequest) {
	try {
      ...
      `);
		...
		if (error) {
			console.error("Error fetching assessment objectives:", error);
		...
		console.error("Error in assessment objectives API:", error);
		return NextResponse.json(
...
export async function POST(request: NextRequest) {
	try {
        ...
        `)
			...
			if (error) {
				console.error("Error fetching batch assessment objectives:", error);
				return NextResponse.json(
					{ error: "Failed to fetch assessment objectives" },
					{ status: 500 },
				);
      ...
      `,
		...
		if (error) {
			console.error("Error fetching assessment objectives stats:", error);
			return NextResponse.json(
		...
		const statsByControl =
			objectives?.reduce(
				(acc, obj) => {
					...
					if (obj.scf_baseline_aos)
						acc[controlId].framework_coverage.scf_baseline++;
					if (obj.dhs_ztcf_aos) acc[controlId].framework_coverage.dhs_ztcf++;
					if (obj.nist_800_53_r5_aos)
						acc[controlId].framework_coverage.nist_800_53_r5++;
					if (obj.nist_800_171_r2_aos)
						acc[controlId].framework_coverage.nist_800_171_r2++;
					if (obj.nist_800_171_r3_aos)
						acc[controlId].framework_coverage.nist_800_171_r3++;
					if (obj.nist_800_172_aos)
						acc[controlId].framework_coverage.nist_800_172++;
			...
			(acc, controlId) => {
				...
				acc[controlId] = {
					...
					coverage_summary: {
						...
						most_covered_framework: Object.entries(
							stats.framework_coverage,
						).reduce(
							(max, [framework, count]) => {
								const countNum = count as number;
								return countNum > max.count
									? { framework, count: countNum }
									: max;
							},
							{ framework: "", count: 0 },
						),
		...
		console.error("Error in assessment statistics API:", error);
		return NextResponse.json(
			{ error: "Internal server error" },
			{ status: 500 },
		);
