...
const log = createLogger("lib/websocket/progress-tracker");
...
class ProgressTracker {
  ...
  private subscribers: Map<string, Set<(update: ProgressUpdate) => void>> = new Map();

  /**
   * Create a new progress session
   */
  createSession(sessionId: string, userId: string, operation: string): ProgressSession {
    ...
    this.broadcastUpdate(sessionId, "initializing", 0, `Starting ${operation}...`);
  ...
  updateProgress(
    ...
    if (!session) {
      log.warn("progress_tracker.session_not_found", {
        detail: `Progress session ${sessionId} not found`,
      });
      return;
    }
   ...
   */
  completeSession(sessionId: string, message: string = "Operation completed"): void {
    const session = this.sessions.get(sessionId);
    if (!session) {
      log.warn("progress_tracker.session_not_found", {
        detail: `Progress session ${sessionId} not found`,
  ...
  errorSession(sessionId: string, error: string): void {
    ...
    this.broadcastUpdate(sessionId, "error", session.progress, `Error: ${error}`);
    ...
    if (subscribers) {
      subscribers.forEach((callback) => {
        try {
          ...
          log.error("progress_tracker.callback_error", {
...
if (typeof window === "undefined" && !globalForProgressTracker.__progressTrackerCleanupInterval__) {
