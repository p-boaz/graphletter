...
export type GraphGapStatus = "compliant" | "partial" | "missing" | "conflicting";
...
function statusToGapType(status: GraphGapStatus, strongestSupportRank: number): string {
...
export type AssessmentVerdict = "pass" | "partial" | "fail";

const verdictToStatus: Record<AssessmentVerdict, GraphGapStatus> = {
  pass: "compliant",
  partial: "partial",
  fail: "conflicting",
};

function buildVerdictSummary(controlId: string, verdict: AssessmentVerdict): string {
  if (verdict === "pass") {
    return `Control ${controlId} passed its latest approved assessment.`;
  }
  if (verdict === "partial") {
    return `Control ${controlId} only partially satisfied its latest approved assessment.`;
  }
  return `Control ${controlId} failed its latest approved assessment — mapped evidence does not satisfy the control.`;
}

/**
 * Overlay approved assessment verdicts onto graph-derived statuses.
 *
 * Mapping strength alone said "evidence exists that maps strongly to this
 * control"; it ignored what the assessment concluded, so FAILed controls
 * surfaced as fully covered (QA 2026-07-09 ISSUE-005). When a control has an
 * approved verdict, that verdict wins: pass → compliant, partial → partial,
 * fail → conflicting. Controls without a verdict keep the graph status.
 */
export function applyAssessmentVerdicts(
  gaps: ControlGapComputation[],
  verdictByControl: Map<string, AssessmentVerdict>
): ControlGapComputation[] {
  return gaps.map((gap) => {
    const verdict = verdictByControl.get(gap.scfControlId);
    if (!verdict) return gap;

    const status = verdictToStatus[verdict];
    if (status === gap.status) return gap;

    return {
      ...gap,
      status,
      gapType: statusToGapType(status, gap.strongestSupportRank),
      summary: buildVerdictSummary(gap.scfControlId, verdict),
    };
  });
}
...
export function computeControlGaps(
  ...
  return controlIds.map((controlId) => {
    ...
    const status = resolveStatus(strongestSupportRank, strongestContradictionRank);
    ...
    return {
      ...
      status,
      gapType,
      summary: buildGapSummary(controlId, status, strongestSupportRank, strongestContradictionRank),
      supportingAtomIds: [
