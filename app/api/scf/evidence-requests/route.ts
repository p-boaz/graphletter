...
const log = createLogger("api/scf/evidence-requests");
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      log.error("evidence_requests.fetch_failed", {
    ...
    log.error("evidence_requests.unhandled_error", {
      detail: error instanceof Error ? error.message : String(error),
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!control_ids || !Array.isArray(control_ids)) {
      return NextResponse.json({ error: "control_ids array is required" }, { status: 400 });
    }
    ...
    if (error) {
      log.error("evidence_requests.stats_fetch_failed", {
        detail: error instanceof Error ? error.message : String(error),
      });
    ...
    log.error("evidence_requests.statistics_unhandled_error", {
      detail: error instanceof Error ? error.message : String(error),
    });
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
