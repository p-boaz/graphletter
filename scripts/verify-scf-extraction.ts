...
async function regenerateLicenseDocs(repoRoot: string, manifest: Manifest): Promise<void> {
  ...
  await writeFile(auditPath, auditFormatted, "utf8");
  ...
  const readmeRaw = `# graphletter \`data/\` license and provenance
...
${rows}
`;
  const readmeFormatted = await prettier.format(readmeRaw, {
    parser: "markdown",
    filepath: readmePath,
  });
  await writeFile(readmePath, readmeFormatted, "utf8");
}
