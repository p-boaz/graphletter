...
// Render the list in pages. An unbounded map rendered ~178 full cards (37k px
// page) and fired one framework-impact request per card — QA 2026-07-09
// ISSUE-002/003.
const REMEDIATIONS_PER_PAGE = 10;
...
export function GapRemediationPanel({
  ...
  const [visibleCount, setVisibleCount] = useState(REMEDIATIONS_PER_PAGE);
  ...
  useEffect(() => {
    ...
    fetchRemediations();
    setVisibleCount(REMEDIATIONS_PER_PAGE);
    ...
    return (
      <Card>
        <CardContent className="flex items-center justify-center gap-2 py-8">
          ...
          <span className="text-slate-500 text-sm">Analyzing gap remediations...</span>
        </CardContent>
      </Card>
    );
  }
    ...
    <Card>
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <CardDescription className="mt-1">
              {data.totalGaps} control{data.totalGaps !== 1 ? "s" : ""} need evidence —{" "}
              {data.remediations.length} artifact
      ...
      <CardContent>
        <div className="space-y-4">
          {data.remediations.slice(0, visibleCount).map((rem) => (
            <div
              key={rem.erlId}
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <p className="font-medium text-slate-900 text-sm">{rem.artifact}</p>
                ...
                {onStartUpload && (
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => onStartUpload(rem.artifact, rem.controlsCovered)}
          ...
          {data.remediations.length > visibleCount && (
              ...
              <Button
                ...
                onClick={() =>
                  setVisibleCount((current) =>
                    Math.min(current + REMEDIATIONS_PER_PAGE, data.remediations.length)
                  )
                }
              >
                Show {Math.min(REMEDIATIONS_PER_PAGE, data.remediations.length - visibleCount)} more
                of {data.remediations.length - visibleCount} remaining
              </Button>
            </div>
          )}
        </div>
