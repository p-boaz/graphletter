...
const log = createLogger("lib/assessment-transformer");
...
export async function transformAssessmentsForReview(
  ...
  const controlIds = Array.from(new Set(rawAssessments.map((a) => a.scf_control_id)));
  ...
  if (controlsError) {
    log.error("assessment_transformer.controls_fetch_failed", {
  ...
  const { data: aoData, error: aoError } = await supabase
    ...
    .select("scf_ao_id, assessment_objective, assessment_procedure, expected_results")
    .in("scf_ao_id", aoIds);

  if (aoError) {
    log.error("assessment_transformer.ao_fetch_failed", {
      detail: aoError instanceof Error ? aoError.message : String(aoError),
    });
  }
  ...
  controlGroups.forEach((assessments, controlId) => {
    ...
    const objectiveResults = assessments.map((assessment) => {
      ...
      return {
        ...
        assessment_objective: aoInfo?.assessment_objective || "Assessment objective not found",
...
function generateControlSummary(
  ...
  const passCount = assessments.filter((a) => a.assessment_result === "pass").length;
  const failCount = assessments.filter((a) => a.assessment_result === "fail").length;
  const partialCount = assessments.filter((a) => a.assessment_result === "partial").length;

  const avgConfidence = Math.round(
    (assessments.reduce((sum, a) => sum + a.confidence_score, 0) / assessments.length) * 100
  );
