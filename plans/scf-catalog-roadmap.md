...
- [x] Stage 7: done 2026-07-11 (PR #54, spec archived) — 15-framework cohort (FedRAMP ×4, GovRAMP ×6, NIST ×5) live in production: 81 supported / 32,646 mappings / 2.73M crosswalk rows, live-proven (truth line, badges, pagination determinism, search), Prod Smoke green, zero customer-data delta (blast-radius gated). Zero app-code change — the metadata-flip mechanism is proven. Stage 8 precondition: non-destructive additive promotion path (spec Follow-ups item 3)
