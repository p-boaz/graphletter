...
export async function gatherReportData(
	...
	const [frameworksResult, evidenceResult, gapsResult, assessmentsResult] =
		await Promise.all([
			supabase
				.from("scf_frameworks")
	...
	const evidenceByType = new Map<
		string,
		{ count: number; latestUpload: string | null }
	>();
	for (const e of (evidenceResult.data || []) as Array<{
		...
		if (existing) {
			...
			if (
				e.submitted_at &&
				(!existing.latestUpload || e.submitted_at > existing.latestUpload)
			) {
	...
	const evidenceSummary: EvidenceSummaryRow[] = Array.from(
	...
	// Process gaps
	const latestAssessmentByControl = new Map<
		string,
	...
	for (const gap of (gapsResult.data || []) as GapAnalysisRow[]) {
		const domainInfo = posture?.domains.find((d) =>
			gap.scf_control_id.startsWith(d.domainId),
		);
		...
		controlDetails.push({
			...
			domainName: sanitizeField(
				"domainName",
				domainInfo?.domainName || "Unknown",
			),
			status: gap.status,
			evidenceCount: 0,
...
export function generateCSVReport(data: ComplianceReportData): string {
	...
	if (data.posture?.domains.length) {
		...
		lines.push(
			"Domain,Tier,Weight,Total Controls,Compliant,Partial,Missing,Raw Score",
		);
	...
	if (data.gapSummary.length > 0) {
		...
		for (const g of data.gapSummary) {
			lines.push(
				[
					csvEscape(g.controlId),
					csvEscape(g.domainId),
					g.status,
					g.gapType,
				].join(","),
			);
