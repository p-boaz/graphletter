...
const log = createLogger("api/ai/gap-analysis");
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!validateAIEnvironment()) {
      return NextResponse.json({ error: "AI services not properly configured" }, { status: 500 });
    }
    ...
    log.error("ai.gap_analysis.unhandled", {
