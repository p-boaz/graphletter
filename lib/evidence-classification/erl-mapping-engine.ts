...
export class ERLMappingEngine {
	...
	static async findBestFitERL(
		...
		switch (classification.artifact_type) {
			...
			case "Multi-factor authentication configuration":
				return ERLMappingEngine.mapMFAEvidence(
					classification,
					mappingEvidence,
					availableERLs,
				);
			...
			default:
				return ERLMappingEngine.mapGenericEvidence(
					classification,
					evidenceData,
					availableERLs,
				);
	...
	private static mapEncryptionConfigurationEvidence(
		...
		if (!primaryERL)
			throw new Error(
				"E-CRY-01 (Cryptographic Protections) not found in ERL list",
			);
		...
		const coverageRatio =
			totalResources > 0 ? protectedResources / totalResources : 0;

		// Assess control satisfaction based on coverage
		const controlAssessment = primaryERL.scf_control_mappings.map(
			(controlId) => {
				if (controlId === "CRY-05") {
					// Encryption at rest
					return {
					...
					return {
						...
						basis:
							"Key management policies and procedures not evidenced by configuration data",
					};
				} else {
					return {
						control_id: controlId,
						...
						basis:
							"Configuration evidence provides supporting but not complete evidence",
					};
				}
			},
		);
		...
		if (keyMgmtERL) {
			complementaryERLs.push({
				...
				reason:
					"Key management policies, rotation, and governance not evidenced",
	...
	private static mapAccessControlEvidence(
		...
		if (!primaryERL)
			throw new Error(
				"E-IAM-01 (Access Permission Review) not found in ERL list",
			);
	...
	private static mapDataProtectionEvidence(
		...
		const primaryERL =
			backupERL || availableERLs.find((erl) => erl.erl_id === "E-AST-11");
	...
	private static mapMFAEvidence(
		...
		if (!primaryERL)
			throw new Error("E-IAM-05 (IAM Function) not found in ERL list");
	...
	private static mapPasswordPolicyEvidence(
		...
		if (!primaryERL)
			throw new Error("E-IAM-06 (AAA Solution) not found in ERL list");
		...
		const policyStrength =
			ERLMappingEngine.assessPasswordPolicyStrength(policy);
			...
			gaps_identified: [
				{
					...
					description:
						"Evidence type not recognized - manual classification needed",
					suggested_artifacts: [],
					priority: "high" as const,
				},
			],
	 ...
	 */
	private static assessPasswordPolicyStrength(
