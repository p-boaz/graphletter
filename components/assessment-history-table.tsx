  ...
  return (
    <div className="space-y-4">
      {assessments.map((assessment) => {
        ...
        const maturityAssessment = assessment.metadata?.maturity_assessment || null;
                    ...
                    <div className="flex flex-1 items-center space-x-4">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="mb-1 flex items-center gap-2">
                          <h4 className="font-medium text-sm">{assessment.scf_control_id}</h4>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {maturityAssessment && (
                          <Badge variant="outline" className="flex items-center gap-1 text-xs">
                            <Gauge className="h-3 w-3" />L{maturityAssessment.assessed_level}
                          </Badge>
                        )}
                      </div>
                  ...
                  <div className="space-y-4 pt-4">
                    ...
                    {assessment.assessment_notes && (
                      <div>
                        ...
                        <p className="text-gray-700 text-sm">{assessment.assessment_notes}</p>
                      </div>
                    )}
                    ...
                    {hasObjectives && (
                      <div>
                        ...
                        <div className="space-y-2">
                          {assessment.metadata?.objective_results?.map((objective, idx) => (
                            <div
                              key={idx}
                            ...
                            >
                              ...
                              <p className="text-gray-700 leading-tight">{objective.reasoning}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {maturityAssessment && (
                      <div>
                        <h5 className="ft-eyebrow mb-2 text-[11px]">Maturity Assessment</h5>
                        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-700 space-y-2">
                          <div className="flex items-center justify-between">
                            ...
                            <Badge variant="outline" className="text-[10px]">
                              {Math.round((maturityAssessment.confidence || 0) * 100)}% confidence
                            </Badge>
                          </div>
                          {typeof maturityAssessment.target_level === "number" && (
                            <div>
                              ...
                              {typeof maturityAssessment.target_met === "boolean" && (
                                <Badge
                                  variant={
                                    maturityAssessment.target_met ? "default" : "destructive"
                                  }
                              ...
                              {typeof maturityAssessment.target_gap === "number" && (
                                <span className="ml-2">
                                  Gap {maturityAssessment.target_gap >= 0 ? "+" : ""}
                                  {maturityAssessment.target_gap}
                                </span>
                              )}
                            </div>
                          )}
                          <p className="leading-relaxed">{maturityAssessment.rationale}</p>
                    ...
                    {/* Linked Evidence */}
                    {assessment.linked_evidence && assessment.linked_evidence.length > 0 && (
                      <div>
                        <h5 className="mb-1 font-medium text-gray-900 text-xs">Linked Evidence</h5>
                        <div className="space-y-1">
                          {assessment.linked_evidence.map((evidence) => (
                            ...
                            >
                              ...
                              <span className="flex-1">{evidence.file_name}</span>
                              <Badge variant="outline" className="text-xs">
