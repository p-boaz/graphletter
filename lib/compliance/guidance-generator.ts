...
function isValidEffort(value: string): value is "low" | "medium" | "high" {
...
export async function generateGuidance(
  ...
  if (cached) {
    ...
    return {
      ...
      exampleSections: Array.isArray(row.example_sections)
        ...
        : [],
      estimatedEffort: isValidEffort(row.estimated_effort) ? row.estimated_effort : "medium",
      cached: true,
  ...
  const cbResult = await checkCircuitBreaker(COMPLIANCE_AI_CONFIG.recommendations.provider);
  if (!cbResult.allowed) {
  ...
  try {
    ...
    supabase
      ...
      .then(
        ...
        (err: Error) => log.warn("Failed to cache guidance", { error: err.message })
      );
