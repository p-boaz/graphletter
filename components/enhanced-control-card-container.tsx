...
export function EnhancedControlCardContainer(props: EnhancedControlCardContainerProps) {
  ...
  if (error) {
    ...
    return (
      <Card className="border-slate-200 bg-white shadow-sm">
        <CardContent className="p-4">
          <div className="text-xs text-red-500 mb-2">Failed to load enhanced data: {error}</div>
          <EnhancedControlCard {...props} risks={[]} threats={[]} maturityLevels={null} />
        </CardContent>
      </Card>
    );
  }
  ...
  if (!data) {
    ...
    return <EnhancedControlCard {...props} risks={[]} threats={[]} maturityLevels={null} />;
  }
