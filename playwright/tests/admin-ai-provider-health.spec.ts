...
test("admin AI provider health API requires authentication", async ({
	request,
}) => {
...
test("admin AI provider health page renders provider rows", async ({
	page,
}, testInfo) => {
	...
	try {
		...
		await expect(
			page.getByTestId("admin-ai-provider-health-page"),
		).toBeVisible();
		await expect(
			page.getByTestId("admin-ai-provider-health-table"),
		).toBeVisible();
		await expect(
			page.getByTestId("admin-ai-provider-health-row-openai"),
		).toContainText("healthy");
		await expect(
			page.getByTestId("admin-ai-provider-health-row-anthropic"),
		).toContainText("tripped");
	} finally {
		observer.stop();
...
test("admin AI provider health page shows forbidden state", async ({
	page,
}, testInfo) => {
	await page.route("**/api/admin/ai-provider-health**", async (route) => {
		await route.fulfill({
	...
	await expect(
		page.getByTestId("admin-ai-provider-health-error"),
