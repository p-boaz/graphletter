...
export default function AnalyticsPage() {
  ...
  const [coverageSummary, setCoverageSummary] = useState<GraphCoverageSummary | null>(null);
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const [evidenceResponse, assessmentResponse, coverageResponse, controlsResponse] =
        await Promise.all([
      ...
      if (coverageResponse.ok) {
        ...
        if (coverageData.success) {
          ...
          setGraphControls(Array.isArray(coverageData.controls) ? coverageData.controls : []);
        }
      }
  ...
  const avgMaturityLevel =
    totalMaturityAssessments > 0
      ? maturityAssessments.reduce((sum, item) => sum + (item.assessed_level || 0), 0) /
        totalMaturityAssessments
      : 0;

  const avgMaturityConfidence =
    totalMaturityAssessments > 0
      ? maturityAssessments.reduce((sum, item) => sum + (item.confidence || 0), 0) /
        totalMaturityAssessments
      : 0;

  const onTargetCount = maturityAssessments.filter((item) => item.target_met === true).length;
  ...
  const graphTotalControls = coverageSummary?.total_controls ?? graphControls.length;
  ...
  const domainCoverageRows = useMemo(() => {
    const metadataByControlId = new Map(controlCatalog.map((control) => [control.id, control]));
    ...
    for (const control of graphControls) {
      ...
      const domainDescription = domainInfo?.description?.trim() || `${domainName} control domain`;
      ...
      .map((row) => ({
        ...
        coverage_percentage:
          row.total_controls > 0
            ? Math.round(
                ((row.compliant_controls + row.partial_controls) / row.total_controls) * 100
              )
            : 0,
      }))
      .sort((a, b) => {
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
              ...
              {totalMaturityAssessments === 0 ? (
                ...
                <>
                  ...
                  <div className="rounded bg-slate-50 p-3 text-slate-600 text-xs">
                    Average maturity confidence: {Math.round(avgMaturityConfidence * 100)}%
                  </div>
                </>
              )}
            </CardContent>
          ...
          <Card className="ft-card" data-testid="analytics-activity-card">
            <CardHeader>
              ...
              <CardDescription className="ft-sans text-base text-slate-600 leading-relaxed">
                How much evidence you&apos;ve processed and what it covers.
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <Card className="ft-card" data-testid="analytics-domain-coverage-card">
          <CardHeader>
            ...
            <CardDescription className="ft-sans text-base text-slate-600 leading-relaxed">
              Per-domain breakdown using the same coverage rules as the Overview dashboard.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {domainCoverageRows.length === 0 ? (
              <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-6 text-center text-slate-600 text-sm">
                No domain coverage data available yet. Upload and map evidence to see domain-level
                coverage.
              </div>
            ) : (
                ...
                <table className="min-w-full border-collapse text-sm">
                  ...
                  <tbody>
                    {domainCoverageRows.map((domain) => (
                      <tr key={domain.domain_id} className="border-slate-100 border-b align-top">
                        ...
                        <td className="px-3 py-3 text-right">
                          <div className="flex items-center justify-end gap-2">
                            <div className="w-24">
                              <Progress value={domain.coverage_percentage} className="h-2" />
                            </div>
        ...
        <Card className="ft-card" data-testid="analytics-export-card">
          <CardHeader>
            ...
            <CardDescription className="ft-sans text-base text-slate-600 leading-relaxed">
              Export audit-ready reports from your current evidence and assessment results.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="rounded bg-gray-50 p-3 text-gray-500 text-xs">
              <p>
                <strong>Coverage snapshot:</strong> {graphControlsWithEvidence}/{graphTotalControls}{" "}
                controls fully or partially covered, {graphCoverageRate}% coverage.
              </p>
              <p>
                <strong>Current gaps:</strong> {graphMissingControls} with no evidence,{" "}
                {graphConflictingControls} conflicting.
              </p>
            </div>
          </CardContent>
        </Card>
