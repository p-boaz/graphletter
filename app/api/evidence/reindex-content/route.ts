...
export async function POST(request: NextRequest) {
	...
	try {
		...
		return NextResponse.json(
			{
				error:
					error instanceof Error ? error.message : "Content reindexing failed",
			},
			{ status: 500 },
		);
	}
	...
	try {
		...
		const summary = {
			...
			pending: statusSummary.filter(
				(s) =>
					!s.content_extraction_status ||
					s.content_extraction_status === "pending",
			).length,
			processing: statusSummary.filter(
				(s) => s.content_extraction_status === "processing",
			).length,
			completed: statusSummary.filter(
				(s) => s.content_extraction_status === "completed",
			).length,
			failed: statusSummary.filter(
				(s) => s.content_extraction_status === "failed",
			).length,
			skipped: statusSummary.filter(
				(s) => s.content_extraction_status === "skipped",
			).length,
		};
