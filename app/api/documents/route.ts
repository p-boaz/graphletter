...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      return apiError("documents.get_failed", "Failed to fetch documents", 500, error);
    }
    ...
    return apiError("documents.get_unexpected_failed", "Failed to fetch documents", 500, error);
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.sourceEvidenceId) {
      ...
      if (!evidence) {
        return NextResponse.json({ error: "Source evidence not found" }, { status: 404 });
      }

      sourceEvidence = evidence as EvidenceRecord;
    }

    const content = body.content ?? (sourceEvidence ? resolveEvidenceContent(sourceEvidence) : "");
    ...
    const baseDocumentPayload = {
      ...
      storage_path:
        body.storagePath ?? sourceEvidence?.storage_path ?? sourceEvidence?.file_path ?? null,
    ...
    return apiError("documents.post_failed", "Failed to create document", 500, error);
  }
}
