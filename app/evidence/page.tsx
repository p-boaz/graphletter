			...
			try {
				...
				if (data.success) {
					const results = Array.isArray(data.results)
						? (data.results as EvidenceApiItem[])
						: [];
	...
	const handleEvidenceOpen = async (
		...
		try {
			const response = await fetch(
				`/api/evidence/download?id=${evidenceId}&action=${action}`,
			);
			const data = await response.json();
				...
				<div className="container mx-auto px-4">
					<div className="text-center space-y-8">
						...
						<p className="ft-sans text-slate-600 text-xl max-w-3xl mx-auto">
							Search uploaded evidence, verify control mappings, and open source
							files for review or export.
						</p>
					</div>
				</div>
			</section>
							...
							<CardContent className="pt-6">
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="ft-serif font-bold text-2xl text-red-600">
											{stats.needsReview}
										</p>
									</div>
										...
										<div className="flex items-center gap-4">
											<div className="flex gap-2">
												...
												<Button
													...
													variant="outline"
													onClick={() =>
														handleEvidenceOpen(item.id, "download")
													}
													disabled={downloadingEvidenceId === item.id}
												>
