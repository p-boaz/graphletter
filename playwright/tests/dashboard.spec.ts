...
test("dashboard smoke flow renders key states and framework focus", async ({
	page,
}, testInfo) => {
	...
	const handleConsole = (msg: { type: () => string; text: () => string }) => {
		if (
			...
			msg
				.text()
				.includes(
					"[AssessmentReviewDialog] No result or empty assessments array",
				)
		) {
	...
	try {
		...
		await expect(
			page.getByTestId(selectors.public.navHeader),
		).not.toContainText("Sign in");
		await expect(
			page.getByTestId(selectors.dashboard.coverageSummaryCard),
		).toContainText("1/2");
		await expect(
			page.getByTestId(selectors.dashboard.coverageSummaryCard),
		).toContainText("100% Coverage");
		await expect(page.getByText("Coverage by Domain")).toHaveCount(0);
		await expect(
			page.getByRole("link", { name: "Open Analytics drill-down" }),
		).toBeVisible();

		await page.getByTestId(selectors.dashboard.frameworkFilterTrigger).click();
		await page
			.getByRole("option", {
				name: "NIST CSF v2.0 (2 mapping links)",
				exact: true,
			})
			.click();

		await expect(
			page.getByTestId(selectors.dashboard.coverageModeBadge),
		).toHaveText("Framework focus mode");
		await expect(
			page.getByTestId(selectors.dashboard.clearFrameworkFocusButton),
		).toBeVisible();
		await page
			.getByTestId(selectors.dashboard.clearFrameworkFocusButton)
			.click();
		await expect(
			page.getByTestId(selectors.dashboard.coverageModeBadge),
		).toHaveText("SCF coverage");
