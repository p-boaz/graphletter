...
export async function exportGraphComplianceReport(format: "csv" | "json") {
  const response = await fetch(`/api/reports/compliance-export?format=${format}&details=true`, {
    method: "GET",
  });

  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throw new Error(payload.error || "Failed to export graph compliance report");
  }
