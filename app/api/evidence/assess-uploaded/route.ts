...
export async function POST(request: NextRequest) {
	...
	try {
		...
		if (!fileContent) {
			return NextResponse.json(
				{ error: "File content required for assessment" },
				{ status: 400 },
			);
		}
		...
		if (evidenceError || !evidenceRecords?.length) {
			return NextResponse.json(
				{ error: "Evidence records not found" },
				{ status: 404 },
			);
		}
		...
		const getAssessmentProgressValue = () =>
			Math.min(
				95,
				65 + Math.round((completedControls / Math.max(totalControls, 1)) * 30),
			);
		...
		for (const [index, controlId] of uniqueControlIds.entries()) {
			...
			const controlEvidenceRecords = evidenceRecords.filter(
				(e) => e.scf_control_id === controlId,
			);
			...
			if (
				...
				if (existingAssessment) {
					const reusedConfidence = confidenceLevelToScore(
						existingAssessment.confidence_level,
					);
					...
					assessmentResults.push({
						...
						summary:
							existingAssessment.assessment_summary ||
					...
					const existingMetadata =
						...
						typeof existingAssessment.metadata === "object"
			...
			try {
				...
				if (!assessment) {
					throw new Error(
						`Assessment produced no result for control ${controlId}`,
					);
				}
