...
function MappingExplorerContent() {
	...
	const [selectedControl, setSelectedControl] = useState<SCFControl | null>(
		null,
	);
	const [selectedControlDetail, setSelectedControlDetail] =
		useState<ControlDetail | null>(null);
	...
	const filterControls = (controlsToFilter: SCFControl[]) => {
		...
		if (selectedDomain !== "all") {
			filtered = filtered.filter(
				(control) => control.domain_id === selectedDomain,
			);
		}
		...
		if (selectedFrameworkName) {
			filtered = filtered.filter((control) =>
				(control.scf_control_mappings || []).some((mapping) => {
					const frameworkName =
						mapping.scf_frameworks?.framework_name?.toLowerCase();
			...
			try {
				...
				if (statsResponse.ok) {
					...
					setStats({
						...
						totalFrameworks:
							statsData.frameworks || statsData.totalFrameworks || 0,
	...
	useEffect(() => {
		...
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [allControls, searchTerm, selectedDomain, selectedFrameworkName]);
	...
	const domains = useMemo(
		() =>
			Array.from(
				new Set(allControls.map((control) => control.domain_id)),
			).sort(),
		[allControls],
	);
	...
	const derivedHasAssessment = Boolean(
		selectedControlDetail?.control_questions &&
			selectedControlDetail.control_questions.length > 0,
	);
		...
		<div className="space-y-6">
			<div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
				<div>
					...
					<h2 className="ft-serif text-3xl font-bold text-slate-900">
						SCF Mapping Explorer
					</h2>
					<p className="ft-sans mt-1 text-slate-600">
						Explore {safeToLocaleString(allControls.length)} controls mapped
						across {stats.totalFrameworks} frameworks.
					</p>
				</div>
			...
			<div className="grid grid-cols-1 gap-4 md:grid-cols-3">
				<Card className="bg-gradient-to-r from-blue-500 to-blue-600 text-white">
					<CardContent className="p-4">
						<div className="flex items-center justify-between">
							<div>
								<p className="text-blue-100 text-sm">Total Controls</p>
				...
				<Card className="bg-gradient-to-r from-emerald-500 to-emerald-600 text-white">
					<CardContent className="p-4">
						<div className="flex items-center justify-between">
							<div>
								<p className="text-emerald-100 text-sm">Frameworks</p>
								<p className="text-2xl font-semibold">
				...
				<Card className="bg-gradient-to-r from-purple-500 to-purple-600 text-white">
					<CardContent className="p-4">
						<div className="flex items-center justify-between">
							<div>
								<p className="text-purple-100 text-sm">Cross-Mappings</p>
								<p className="text-2xl font-semibold">
							...
							{selectedControl ? (
								detailLoading ? (
									...
									<div className="space-y-3">
										{selectedControlDetail.scf_control_mappings.map(
											(mapping) => (
												<div
													key={mapping.id}
												...
												>
													<div className="flex items-center justify-between">
														...
														<Badge
															variant={
																(mapping.confidence_score || 0) >= 0.8
																	? "default"
																	: "secondary"
															}
															className="text-xs"
														>
															{Math.round(
																(mapping.confidence_score || 0) * 100,
															)}
															%
														</Badge>
													</div>
