...
function normalizeDomainName(raw: ControlCatalogRow["scf_domains"]): string | null {
  if (!raw) return null;
...
export async function calculatePostureScore(
  ...
  if (tierError || !tierRows?.length) {
    ...
    weightMap = new Map(
      (tierRows as TierWeightRow[]).map((r) => [r.domain_id, { tier: r.tier, weight: r.weight }])
    );
  }

  // 2. Fetch gap analysis data for user. Paginated: an unpaginated select
  ...
  // 3. Fetch control catalog for domain mapping. Chunked: a single .in()
  ...
  for (let i = 0; i < controlIds.length; i += CATALOG_CHUNK) {
    ...
    for (const row of (catalogRows || []) as ControlCatalogRow[]) {
      controlDomainMap.set(row.id, row);
    }
  }
  ...
  for (const [domainId, agg] of domainAgg) {
    ...
    const tier = (tierInfo?.tier as "critical" | "high" | "standard") ?? "standard";
  ...
  const overallScore =
    totalWeight > 0 ? Math.round((totalWeightedScore / totalWeight) * 10000) / 100 : 0;
 ...
 */
export function enqueuePostureRecalc(supabase: SupabaseClient, userId: string): void {
