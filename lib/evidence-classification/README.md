...
await EvidenceProcessor.processEvidence(
  userId, connectionId, evidenceData, dataSource, checkType, 
  processedContent, supabase, evidenceRecords, assessments, sessionId
);
```
...
- **Azure**: Add `classifyAzureEvidence()` rules
- **GCP**: Add `classifyGCPEvidence()` rules  
