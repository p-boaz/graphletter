...
function getOrCreateWindowState(
	...
	const windowExpired =
		!existingState || nowMs - existingState.windowStart >= windowMs;
...
function checkFixedWindowRateLimitInternal(
	...
	const windowState = getOrCreateWindowState(
		store,
		compositeKey,
		nowMs,
		options.windowMs,
...
declare global {
	var __fixedWindowRateLimitStore__:
		| Map<string, RateLimitWindowState>
		| undefined;
...
const fixedWindowRateLimitStore =
...
if (
	...
	globalRateLimitState.__fixedWindowRateLimitCleanupInterval__ = setInterval(
		() => {
			...
			cleanupFixedWindowRateLimitStore(
				fixedWindowRateLimitStore,
				6 * 60 * 60 * 1000,
			);
		},
