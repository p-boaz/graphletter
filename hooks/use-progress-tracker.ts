...
export interface ProgressState {
  ...
  progress: number;
  message: string;
  status: "connecting" | "connected" | "active" | "completed" | "error" | "disconnected";
...
export function useProgressTracker(options: UseProgressTrackerOptions = {}) {
  const { onProgress, onComplete, onError, autoReconnect = true, reconnectDelay = 5000 } = options;
  ...
  const connect = useCallback(
    (sessionId: string) => {
      ...
      try {
        const eventSource = new EventSource(`/api/ws/progress?sessionId=${sessionId}`);
   ...
   */
  const isTracking = progressState.status === "active" || progressState.status === "connected";

  /**
   * Check if connection is established
   */
  const isConnected = progressState.status === "connected" || progressState.status === "active";
...
export function useProgressSession(options: UseProgressTrackerOptions = {}) {
  ...
  const endSession = useCallback(
    async (options: { status?: "completed" | "error"; message?: string } = {}) => {
      if (sessionId) {
        if (options.status) {
          try {
