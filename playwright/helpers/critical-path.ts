...
const DEFAULT_FIXTURE_PATH = path.resolve(process.cwd(), "data/anthropic-controls.pdf");

export async function runCriticalPath(page: Page, options: CriticalPathOptions): Promise<void> {
  ...
  await page.getByPlaceholder("Search artifacts...").fill(options.artifactName);
  await page
