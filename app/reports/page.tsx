...
import { BarChart3, Download, FileText, Filter, Search, TrendingUp } from "lucide-react";
...
const describeEvidenceStrength = (rank: number): string => {
  if (rank >= 4) return "Based on strong supporting evidence.";
  if (rank >= 3) return "Based on moderate supporting evidence.";
  if (rank >= 2) return "Based on weak supporting evidence.";
...
export default function ReportsPage() {
  ...
  const exportComplianceReport = async (format: "csv" | "json") => {
    ...
    try {
      ...
      setReportsError(error instanceof Error ? error.message : "Failed to export report");
    } finally {
  ...
  useEffect(() => {
    const loadReports = async () => {
      ...
      try {
        ...
        const mappedReports: ReportItem[] = ((data.controls || []) as GraphCoverageControl[]).map(
          (control) => ({
            ...
            description: describeEvidenceStrength(control.strongest_coverage_rank),
          })
        );
  ...
  const availableFrameworks = Array.from(new Set(reports.map((report) => report.framework))).sort();
  const availableTypes = Array.from(new Set(reports.map((report) => report.type))).sort();
  ...
  const getTypeColor = (type: string) =>
    type === "coverage" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800";
        ...
        <div className="container mx-auto px-4">
          <div className="text-center space-y-8">
            ...
            <p className="ft-sans text-slate-600 text-xl max-w-3xl mx-auto">
              Review completed assessments, track compliance outcomes, and export report data for
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="ft-sans text-slate-600 text-sm">Total Reports</p>
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="ft-serif font-bold text-2xl text-green-600">{stats.published}</p>
                  </div>
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="ft-serif font-bold text-2xl text-yellow-600">{stats.draft}</p>
                  </div>
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="ft-serif font-bold text-2xl text-slate-900">{stats.avgScore}%</p>
                  </div>
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                      <div className="flex-1 space-y-3">
                        <div className="flex items-start justify-between">
                          <div className="space-y-2">
                            <div className="flex items-center gap-3">
                              <h3 className="ft-serif font-semibold text-lg">{report.title}</h3>
                              <Badge className={getTypeColor(report.type)} variant="secondary">
                                {report.type.charAt(0).toUpperCase() + report.type.slice(1)}
                              </Badge>
                            </div>
                            <p className="ft-sans text-slate-600 text-sm">{report.description}</p>
                          </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                          <div>
                            ...
                            <span className="ml-1 text-slate-600">{report.framework}</span>
                          </div>
                          <div>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-center">
                          ...
                          <div className="ft-sans text-xs text-slate-500">Score</div>
                        </div>
                      </div>
                    </div>
                  </CardContent>
