      ...
      `)
		...
		if (evidenceIds.length > 0) {
			...
			if (evidenceLookupError) {
				console.warn(
					"Failed to load linked evidence for assessment history:",
		...
		const { data: statsData } = await supabase
			.from("assessments")
			.select(
				"assessment_result, assessment_status, scf_control_id, completed_at",
			)
			.eq("user_id", user.id);
		...
		const stats = {
			...
			by_result:
				(statsData as AssessmentStatsRow[] | null)?.reduce(
					(acc: Record<string, number>, item) => {
						if (item.assessment_result) {
							acc[item.assessment_result] =
								(acc[item.assessment_result] || 0) + 1;
						}
						return acc;
					},
					{},
			...
			by_status:
				(statsData as AssessmentStatsRow[] | null)?.reduce(
					(acc: Record<string, number>, item) => {
						acc[item.assessment_status] =
							(acc[item.assessment_status] || 0) + 1;
						return acc;
					},
					{},
				) || {},
		...
		console.error("Error in assessment history API:", error);
		return NextResponse.json(
