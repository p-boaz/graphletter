	...
	return (
		<>
			<div className="rounded-lg bg-yellow-50 border border-yellow-200 p-4 text-center">
				...
				<h3 className="mb-1 font-medium text-yellow-900">Review Required</h3>
			...
			<Card>
				<CardHeader className="pb-3">
					<CardTitle className="flex items-center gap-2 text-base">
						{result.source.type === "evidence" ? "📄" : "🤖"}{" "}
						{result.source.name}
					</CardTitle>
					<CardDescription>
						Assessment completed • {getOverallScore(result)}% weighted overall
			...
			<Card>
				<CardHeader className="pb-3">
					<CardTitle className="flex items-center gap-2 text-base">
						🎯 AI Assessment Results ({result.assessments.length} controls)
					</CardTitle>
					<CardDescription>
						Review each assessment before approving
					</CardDescription>
				</CardHeader>
				<CardContent>
						...
						<div className="flex flex-wrap gap-3 text-xs">
							{distributionSegments.map((segment) => (
								<span
									...
									className={`font-medium ${segment.textClass}`}
								>
					...
					<div className="space-y-2 max-h-64 overflow-y-auto">
						{result.assessments.map((assessment) => {
							...
							const { topGap, topRecommendation } =
								getTopGapAndRecommendation(assessment);
							const isFailedOrPartial =
								assessment.overall_result === "fail" ||
								assessment.overall_result === "partial";
								...
								>
									<div className="flex items-start space-x-2">
										...
										<div>
											...
											{process.env.NODE_ENV !== "production" && (
												<a
													href={`/api/ai-assessment-logs?control_id=${encodeURIComponent(
														assessment.scf_control_id,
													)}&limit=50`}
													target="_blank"
													rel="noreferrer"
													className="mt-1 inline-flex items-center gap-1 text-blue-700 text-xs hover:underline"
												>
													View AI Logs
									...
									<div className="flex flex-col items-end gap-1.5">
										...
										{assessment.maturity_assessment && (
											<Badge
												variant="outline"
												className="flex items-center gap-1 border-purple-300 text-purple-700 text-xs"
											>
