...
export default async function FrameworkDetailPage({
	...
	const [frameworkResult, mappingsResult] = await Promise.all([
		supabase
			.from("scf_frameworks")
			.select(
				"id, framework_name, framework_version, total_mappings, mapping_type, scf_version",
			)
			.eq("id", id)
			.single(),
		supabase
				...
				<div className="ft-container">
					<div className="mb-6 max-w-3xl">
						...
						<p className="ft-sans mt-2 text-slate-600">
							Sample mapped controls for this framework. Use these to trace
							requirements back to SCF evidence and objective-level assessments.
						</p>
					</div>

					<div
						className="grid gap-4 md:grid-cols-2"
						data-testid="framework-detail-mappings"
					>
								...
								<Card key={mapping.id} className="h-full border-slate-200">
									<CardHeader className="space-y-2">
										<div className="inline-flex items-center gap-2 text-xs text-slate-500">
											...
											<span>
												{mapping.framework_control_id || mapping.control_id}
											</span>
										</div>
									...
									<CardContent>
										<p className="ft-sans text-sm text-slate-600 leading-relaxed">
											{control?.description ||
												"No control description available for this mapping."}
										</p>
									</CardContent>
								</Card>
							);
					...
					{mappings.length === 0 && (
						<div className="rounded-xl border border-slate-200 bg-slate-50 p-8 text-center">
							...
							<p className="ft-sans mt-2 text-slate-600">
								This framework currently has no associated control mappings in
								the indexed dataset.
							</p>
						</div>
					)}
