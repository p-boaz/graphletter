	...
	try {
		...
		if (!id) {
			return NextResponse.json(
				{ error: "Assessment objective ID is required" },
				{ status: 400 },
			);
		}
      ...
      `)
		...
		if (error) {
			...
			console.error("Error fetching assessment objective:", error);
			return NextResponse.json(
				{ error: "Failed to fetch assessment objective" },
				{ status: 500 },
			);
		}
		...
		console.error("Error in assessment objective API:", error);
		return NextResponse.json(
