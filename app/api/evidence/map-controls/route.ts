...
export async function POST(request: NextRequest) {
  try {
    ...
    if (scfControlIds.length === 0) {
      return NextResponse.json({ error: "At least one scfControlId is required" }, { status: 400 });
    }
    ...
    if (atomIds.length === 0) {
      return NextResponse.json({ error: "No atoms found to map" }, { status: 400 });
    }
    ...
    return apiError("evidence.map_controls_failed", "Failed to map controls", 500, error);
  }
}
