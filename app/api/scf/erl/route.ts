...
const log = createLogger("api/scf/erl");
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      log.error("erl.fetch_failed", {
        detail: error instanceof Error ? error.message : String(error),
      });
      return NextResponse.json({ error: "Failed to fetch evidence requirements" }, { status: 500 });
    }

    // Transform the data to ensure consistent format
    const transformedItems = ((erlItems || []) as (ERLListRow | ERLFilteredRow)[]).map((item) => {
      if (controlId) {
        ...
        return {
          ...
          documentation_artifact: filteredItem.scf_evidence_request_list.documentation_artifact,
          artifact_description: filteredItem.scf_evidence_request_list.artifact_description,
        ...
        return {
          ...
          scf_control_mappings:
            listItem.scf_control_evidence_mappings?.map((m) => m.scf_control_id) || [],
