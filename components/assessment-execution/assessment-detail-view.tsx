...
export function AssessmentDetailView({
	...
	const removeDeficiency = (index: number) => {
		setFormData((prev) => ({
			...prev,
			deficiencies_identified: prev.deficiencies_identified.filter(
				(_, i) => i !== index,
			),
		}));
	};
				...
				<TabsContent value="overview" className="space-y-4">
					<div className="grid grid-cols-2 gap-4 text-sm">
						...
						<div>
							...
							<div className="text-slate-600 capitalize">
								{assessment.assessment_frequency}
							</div>
						</div>
					</div>
								...
								<div>
									...
									<div className="text-slate-600">
										{assessment.assessment_result ? (
											<Badge
												className={getResultColor(assessment.assessment_result)}
											>
								...
								assessment.deficiencies_identified.length > 0 && (
									<div>
										...
										<ul className="mt-1 list-inside list-disc space-y-1 text-slate-600 text-sm">
											{assessment.deficiencies_identified.map(
												(deficiency, index) => (
					...
					<div>
						...
						assessment.evidence_assessment_links.length > 0 ? (
							<div className="mt-2 space-y-2">
								{assessment.evidence_assessment_links.map((link) => (
									<div
										key={link.id}
									...
									>
										<div className="flex items-center gap-2">
											...
											<span className="font-medium text-sm">
												{link.user_evidence.file_name}
											</span>
											<Badge variant="outline" className="text-xs">
												{link.user_evidence.evidence_type}
											</Badge>
											<Badge
												className={`text-xs ${getStatusColor(link.user_evidence.evidence_status)}`}
											>
												{link.user_evidence.evidence_status.replace("_", " ")}
											</Badge>
										</div>
										<div className="flex items-center gap-2">
											<span className="text-slate-500 text-xs">
												{Math.round(link.relevance_score * 100)}% relevant
											</span>
