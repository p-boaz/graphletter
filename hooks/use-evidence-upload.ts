		...
		async (filePath: string, filename?: string) => {
			...
			try {
				...
				if (result.success && result.url) {
					...
					link.download =
						filename || filePath.split("/").pop() || "evidence-file";
					link.target = "_blank";
...
export function useEvidenceManager(userId: string) {
	type EvidenceFile = Awaited<
		ReturnType<(typeof evidenceStorage)["listUserEvidence"]>
	>[number];
	...
	const deleteFile = useCallback(async (filePath: string) => {
		try {
			...
			if (result.success) {
				setFiles((prev) =>
					prev.filter((file) => !filePath.endsWith(file.name)),
				);
