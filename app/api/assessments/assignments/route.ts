...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingAssignment) {
      return NextResponse.json(
        {
          error: "User already has an active assignment of this type for this assessment",
        },
        { status: 400 }
      );
    }
    ...
    if (assignment_type === "primary") {
      await supabase.from("assessments").update({ assigned_to }).eq("id", assessment_id);
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (assignmentCheck.assigned_to !== user.id && assignmentCheck.assigned_by !== user.id) {
      return NextResponse.json(
    ...
    if (assignment_status) {
      ...
      if (assignment_status === "accepted" && assignmentCheck.assignment_status === "assigned") {
