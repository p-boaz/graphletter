...
export interface EvidenceFreshness {
  ...
  ruleSource: "user_override" | "framework_rule" | "type_default" | "global_default";
}
...
export async function scanEvidenceFreshness(
  ...
  const { data: evidenceRows, error: evidenceError } = await supabase
    .from("evidence")
    .select("id, file_name, evidence_type, scf_control_id, submitted_at, evidence_status")
    .eq("user_id", userId)
    .in("evidence_status", ["approved", "completed", "submitted", "under_review"]);
  ...
  const [rulesResult, overridesResult] = await Promise.all([
    supabase.from("evidence_freshness_rules").select("evidence_type, framework_id, max_age_days"),
    supabase
  ...
  const overrideMap = new Map(overrides.map((o) => [o.evidence_id, o.expires_at]));
...
export async function getEvidenceFreshnessStatus(
  ...
  if (override) {
    const days = Math.ceil(
      (new Date(override.expires_at).getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
    );
  ...
  const resolved = resolveMaxAgeDays((rules as FreshnessRule[] | null) || [], evidenceType);
