export function getFrameworkDescription(frameworkName: string): string {
  ...
  if (normalizedName.includes("hipaa")) {
    return "Healthcare privacy and security safeguards for protecting patient health information (PHI) across administrative, physical, and technical controls.";
  }
  ...
  if (normalizedName.includes("iso 27001")) {
    return "Information security management system (ISMS) requirements for risk treatment, control design, and continual security improvement.";
  }
  ...
  return `${frameworkName} requirements, mapped to SCF controls so your evidence counts toward this framework automatically.`;
}
