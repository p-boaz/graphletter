...
export const metadata: Metadata = { title: pageTitle("How it works") };
import {
  analysisLayers,
  explainerIntro,
  glossaryTerms,
...
async function getScfStats(): Promise<ScfStats> {
  try {
    ...
    console.error("Unable to load SCF stats for how-it-works page:", error);
    return null;
  }
}
    ...
    <TooltipProvider>
      <div className="min-h-screen bg-white">
        ...
        <section className="bg-gradient-to-br from-ft-cream to-white">
          <div className="ft-container py-20">
            <div className="max-w-4xl space-y-5">
              <h1 className="ft-headline text-4xl text-ft-black lg:text-5xl">How It Works</h1>

              <div
                className="mt-6 rounded-2xl border border-ft-pink/30 bg-gradient-to-br from-ft-cream to-white p-6 shadow-sm"
                data-testid="hiw-tldr"
              >
                ...
                <p className="mt-3 text-lg text-slate-800 leading-relaxed">
                  <strong>Upload</strong> an evidence document.{" "}
                  <strong>Graphletter reads it</strong> against the{" "}
                  <GlossaryTooltip k="scf">Secure Controls Framework</GlossaryTooltip> using AI,
                  then <strong>maps the result</strong> to every regulatory framework that
                  references those controls. Out comes a per-control pass / partial / fail verdict
                  with reasoning and a gap list.
                </p>
                <p className="mt-3 text-sm text-slate-600 leading-relaxed">
                  Under the hood we build an{" "}
                  <GlossaryTooltip k="erl">Evidence Requirement List</GlossaryTooltip> for each
                  control, extract <GlossaryTooltip k="atom">Evidence Atoms</GlossaryTooltip> from
                  your document, compute a <GlossaryTooltip k="mapping">Mapping</GlossaryTooltip>{" "}
                  from SCF to external frameworks, roll the atoms up into{" "}
                  <GlossaryTooltip k="coverage">Coverage</GlossaryTooltip>, and score implementation{" "}
                  <GlossaryTooltip k="maturity">Maturity</GlossaryTooltip> on a 1–5 scale.
                </p>
                <Button asChild className="mt-4" data-testid="hiw-primary-cta">
        ...
        <section className="ft-container">
          <section
            className="mt-16 rounded-2xl border border-ft-pink/30 bg-gradient-to-br from-ft-cream to-white p-8 text-center"
            data-testid="hiw-closing-cta"
          >
            ...
            <div className="mt-5 flex justify-center gap-3">
              <Button asChild>
                <Link href="/try-it-out">Try the demo</Link>
              </Button>
              <Button asChild variant="outline">
                <Link href={authUrl("signup")}>Sign up free</Link>
              </Button>
            </div>
          </section>
        </section>
