...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const skipContentExtraction = formData.get("skip_content_extraction") === "true";
    ...
    if (!file || !scfControlId || !erlId || !evidenceType) {
      ...
      return NextResponse.json(
        {
          error: "Missing required fields: file, scf_control_id, erl_id, evidence_type",
        },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      const version =
        existingEvidence && existingEvidence.length > 0 ? existingEvidence[0].version + 1 : 1;
      ...
      const evidenceRecord = {
        ...
        content_extracted_at:
          contentExtractionStatus === "completed" ? new Date().toISOString() : null,
        metadata: {
          ...
          content_extraction: {
            ...
            extracted_at: contentExtractionStatus === "completed" ? new Date().toISOString() : null,
      ...
      if (evidenceError) {
        ...
        await serviceSupabase.storage.from("compliance-documents").remove([finalFilePath]);
      ...
      try {
        ...
        logger.warn("evidence.post.assessment_failed", {
          message: assessmentError instanceof Error ? assessmentError.message : "unknown",
        });
    ...
    return apiError("evidence.post_failed", "Failed to upload evidence", 500, error);
  }
}
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    if (evidenceCheck.user_id !== user.id) {
      return NextResponse.json({ error: "Unauthorized to update this evidence" }, { status: 403 });
    }
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (evidence.evidence_status === "approved") {
      return NextResponse.json(
        {
          error: "Cannot delete approved evidence. Please contact an administrator.",
        },
        { status: 400 }
      );
    }
