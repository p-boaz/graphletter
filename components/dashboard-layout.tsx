...
import { EvidenceSummaryCard } from "@/components/CampaignSuccessCard";
...
export function DashboardLayout({
	...
	const uniqueControlsWithEvidence = [
	...
	const passedControls = controlResults.filter(
		(c) => c.overallResult === "pass",
	).length;
			...
			<div className="ft-container space-y-8 py-8">
				...
				<div className="flex items-center justify-between">
					<div>
						...
						<p className="ft-sans mt-2 text-slate-600 text-xl leading-relaxed">
							{description}
						</p>
					</div>
					<div className="flex items-center gap-3">
						{showUploadButton && (
							<SmartEvidenceUpload onEvidenceProcessed={loadEvidenceData} />
						)}
						{/* <DropdownMenu>
					...
					<div className="ft-grid grid-cols-1 gap-12 md:grid-cols-4 mb-12 px-2 md:px-6">
						...
						<EvidenceSummaryCard
							...
							percentage={Math.round(
								(passedControls / (totalAssessedControls || 1)) * 100,
							)}
						...
						<EvidenceSummaryCard
							...
							percentage={Math.round(
								(recentUploadGroups / (totalEvidenceFiles || 1)) * 100,
							)}
						...
						<EvidenceSummaryCard
							...
							percentage={Math.round(
								(uniqueControlsWithEvidence / (totalAssessedControls || 1)) *
									100,
							)}
