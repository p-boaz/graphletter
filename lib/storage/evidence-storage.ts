...
const log = createLogger("lib/storage/evidence-storage");
...
export class EvidenceStorage {
  ...
  async uploadEvidence(
    ...
    try {
      ...
      const finalFilename = filename || `evidence_${timestamp}.${fileExtension}`;
  ...
  async uploadJsonEvidence(
    ...
    try {
      ...
      if (error) {
        log.error("evidence_storage.json_upload_failed", {
   ...
   */
  async getDownloadUrl(filePath: string, expiresIn: number = 3600): Promise<DownloadResult> {
    try {
      const { data, error } = await this.supabase.storage
        .from(this.bucketName)
  ...
  async listUserEvidence(userId: string) {
    try {
      ...
      log.error("evidence_storage.list_user_evidence_failed", {
        detail: error instanceof Error ? error.message : String(error),
      });
      return [];
    }
  ...
  async getFileInfo(filePath: string) {
    try {
      ...
      log.error("evidence_storage.get_file_info_failed", {
        detail: error instanceof Error ? error.message : String(error),
      });
      return null;
    }
  ...
  async copyEvidence(fromPath: string, toPath: string): Promise<UploadResult> {
    try {
      const { error } = await this.supabase.storage.from(this.bucketName).copy(fromPath, toPath);
  ...
  async getUserStorageStats(userId: string) {
    try {
      ...
      log.error("evidence_storage.get_storage_stats_failed", {
        detail: error instanceof Error ? error.message : String(error),
      });
      return {
        totalFiles: 0,
